...
# --------------------------
st.set_page_config(
    page_title="IoT Dashboard Avanzado",
    layout="wide"
)

st.markdown(
    "<h1 style='text-align:center;'>🌿 IoT Dashboard - Monitoreo Ambiental y Movimiento</h1>",
...
if actualizar:
    ...
    if not dht22_data.empty:
        ...
        # Gráficas
        fig_temp = px.line(dht22_data, x="_time", y="temperature", title="Temperatura", markers=True)
        ...
        # Tarjetas con alertas
        col1, col2 = st.columns(2)
        col1.markdown("**Temperatura Actual**")
        tarjeta_valor("Temperatura", dht22_data['temperature'].iloc[-1], "°C", min_val=15, max_val=30)
        col2.markdown("**Humedad Actual**")
        tarjeta_valor("Humedad", dht22_data['humidity'].iloc[-1], "%", min_val=30, max_val=70)

    if not mpu6050_data.empty:
        st.subheader("🌀 Movimiento (MPU6050)")
        fig_acc = px.line(mpu6050_data, x="_time", y=["acc_x","acc_y","acc_z"], title="Aceleración", markers=True)
        st.plotly_chart(fig_acc, use_container_width=True)

        # Tarjetas aceleración
